...
class LightManager:
    ...
    def update_brightness_for_ambient_light(self, pixels):
        ...
        if current_light < 30:  # Very dark
            target_brightness = 0.02  # Very dim but still visible
        elif current_light < 60:  # Dark
            target_brightness = 0.05  # Dim
        elif current_light < 100:  # Indoor lighting
            target_brightness = 0.1  # Low brightness
        elif current_light < 150:  # Bright indoor
            target_brightness = 0.15  # Medium brightness
        elif current_light < 200:  # Very bright indoor
            target_brightness = 0.2  # Higher brightness
        ...
        if abs(target_brightness - current_brightness) > 0.01:
            # Faster transitions when going to/from complete darkness
            if target_brightness == 0.0 or current_brightness == 0.0:
                step_size = 0.05  # Larger steps for on/off transitions
            else:
                step_size = 0.01  # Normal step size for gradual adjustments
    ...
    def check_light_interaction(self):
        ...
        if current_time - self.last_light_update > 0.1:
            ...
            if len([x for x in self.light_history if
                    x > 0]) >= 3:  # Have enough history
